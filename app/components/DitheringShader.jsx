...
// WebGL2 dithering shader, ported from the designali-in snippet to
// plain JSX. Background colour supports an alpha channel (pass
// `#RRGGBBAA` or set the colorBackAlpha prop to 0) so the front
// colour can render alone on whatever surface sits behind the
// canvas — no forced black plate.
//
// The canvas auto-sizes to its wrapper via ResizeObserver, so it
// fills whatever section it's dropped into (the Footer wave, etc.).
...
// Accepts #RRGGBB or #RRGGBBAA; falls back to opaque black.
function hexToRgba(hex) {
  if (typeof hex !== "string") return [0, 0, 0, 1];
...
function compile(gl, source, type) {
  ...
  if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
    console.error("dithering: shader compile", gl.getShaderInfoLog(shader));
...
export default function DitheringShader({
  ...
  useEffect(() => {
    ...
    if (!gl) {
      console.error("dithering: WebGL2 not supported");
      return;
    }
    ...
    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
      console.error("dithering: link", gl.getProgramInfoLog(program));
      return;
    }
    ...
    const render = () => {
      ...
      if (u.u_time) gl.uniform1f(u.u_time, tSec);
      if (u.u_resolution)
        gl.uniform2f(u.u_resolution, canvas.width, canvas.height);
      if (u.u_colorBack) gl.uniform4fv(u.u_colorBack, hexToRgba(colorBack));
      if (u.u_colorFront) gl.uniform4fv(u.u_colorFront, hexToRgba(colorFront));
      if (u.u_shape) gl.uniform1f(u.u_shape, SHAPES[shape] ?? SHAPES.wave);
      if (u.u_type) gl.uniform1f(u.u_type, TYPES[type] ?? TYPES["8x8"]);
      if (u.u_pxSize) gl.uniform1f(u.u_pxSize, pxSize);
      gl.drawArrays(gl.TRIANGLES, 0, 6);
      if (speed !== 0) rafRef.current = requestAnimationFrame(render);
    };
